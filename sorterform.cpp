...
void SorterForm::selectData(QMap<QString, QList<QPair<double, double> > > dataMap)
{
    if(!dataMap.isEmpty()){
        for(double i = ui->tempMinDoubleSpinBox->value(); i <= ui->tempMaxDoubleSpinBox->value(); i += ui->tempRangeDoubleSpinBox->value()){
            ...
            QFile outFile("../results/" + month + "_T_" + QString::number(i) + ".csv");
...
void SorterForm::plotWithGnuPlot(QString plotFile)
{
    if(!plotFile.isEmpty()){
        ...
        outName.remove(".csv").append("_scarce.ps");
        QString com = QString("reset;set terminal postscript landscape enhanced color font 'Times-Roman,14';set output '%1';filename = '%2';"
                              ...
                              "set style line 3 lc rgb 'magenta' pt 7 ps 1.0 lt 1 lw 1.0;set style line 4 lc rgb 'magenta' pt 7 ps 1.0 lt 2 lw 1.0;"
                              ...
                              "set ylabel 'number of occurrence';set key left top;plot filename u 1:2 w l ls 1 title 'wind 1 km/h', "
                              " '' u 1:4 w l ls 3 title 'wind 3 km/h', '' u 1:7 w l ls 5 title 'wind 6 km/h', '' u 1:9 w l ls 7 title 'wind 8 km/h',"
                              " '' u 1:11 w l ls 9 title 'wind 10 km/h';").arg(outName).arg(plotFile);
        Gnuplot gp;
